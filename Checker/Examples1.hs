...
data TState = TState { _tsEnv :: Env, _tsCnt :: Int }
...
inferTypePony :: TM Type -> TM Kind
inferTypePony tmt = do
  t <- tmt
  env <- getEnv
  inferType env t

inferTermPony :: TM Term -> TM Type
inferTermPony tmm = do
  m <- tmm
  env <- getEnv
  inferTerm env m

checkTermPony tmm tmt = do
  t <- tmt
  m <- tmm
  env <- getEnv
  checkTerm env m t
...
(.->.) :: Type -> Type -> TM Type
p .->. q = do { n <- wildTick; return $ Tall n p q }

(!->!) :: TM (Type -> Type -> Type)
(!->!) = Tall <$> wildTick

(!->) :: TM Type -> Type -> TM Type
p !-> q = -- ($ q) <$> (!->!) <*> p
   (!->!) <*> p <*> pure q

(->!) :: Type -> TM Type -> TM Type
p ->! q = (!->!) <*> pure p <*> q

(.=>.) :: TM Type -> TM Type -> TM Type
p .=>. q = Tall <$> wildTick <*> p <*> q
...
neg,negneg :: Type -> TM Type
neg a = a .->. Tbot
negM :: TM Type -> TM Type
negM = (neg =<<) 
negneg x = neg x >>= neg
...
e07 = do
  ...
  p <- predicate "P" [o]
  let pe x = Tapp p x
  -- let py = Tapp p (Mvar "y")      
  f07 <- (hall "x" o  pe) .->. (hall "y" o pe)
  inferTypeTM f07 
...
e12 = do
  ...
  p <- predicate "P" [o]
  let pe x = Tapp p x 
  let f12a = (hexi "x" o (\x  -> unot $ pe x))
  let f12b = hall "x" o pe
  f12c <- neg f12b
  -- f12 <- (hexi2 "x" o (\x -> neg $ pe x)) .=>. (neg $ hall "x" o pe)
  f12 <- f12a .->. f12c
  inferTypeTM f12
